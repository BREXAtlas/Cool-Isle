...
4. A hard shared ceiling of 300 upstream attempts per UTC day will apply before every call. Failed attempts count. There will be no automatic retry of Weather DataHub calls and no retry after `429`.
5. The quota ledger and last-success metadata will be restored from the latest deployed status data and a GitHub Actions cache. Scheduled runs will be serialised. If state is missing, invalid, stale in an unsafe way, or fewer than twelve calls remain, the run will make no official calls and will preserve the last valid dataset.
